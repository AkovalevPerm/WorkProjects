namespace Synchronization.Utils
{
    ...
    public static class AuditTools
    {
        ...
        private static readonly View ViewForEntity = new View(
            new ViewAttribute("ViewForEntity", new[]
            {
        ...
        public static DateTime? GetAuditEntityOperationTimeByPK(Guid pk)
        {
            try
            {
                var lcs = LoadingCustomizationStruct.GetSimpleStruct(typeof(AuditEntity), ViewForEntity);
                lcs.LimitFunction = FunctionBuilder.BuildEquals(pk);
                return DataServiceFactory.AppDataService.LoadObjects(lcs).Cast<AuditEntity>().FirstOrDefault()?.OperationTime;
            }
            catch (Exception ex)
            {
                throw new Exception($"Ошибка при попытке вычитать связанное с SyncEntity изменение аудита(AuditEntity:{pk}). {ex.Message}");
            }
        }
